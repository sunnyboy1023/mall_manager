    ...
    <div>
        ...
        <el-table :data="usersList" stripe style="width: 100%">
            ...
            <el-table-column label="用户状态" width="200">
                <template slot-scope="scope">
                    <el-switch v-model="scope.row.mg_state" active-color="#13ce66" inactive-color="#ff4949" @change="switchChange(scope.row)">
                    </el-switch>
                </template>
            </el-table-column>
            <el-table-column label="操作">
                <template slot-scope="scope">
                    ...
                    <el-button type="success" size="mini" icon="el-icon-check" plain circle @click.prevent="roleDialog(scope.row.id)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        <div class="block">
        ...
        <el-form :model="form">
            ...
            <el-form-item label="分配角色" label-width="80px">
                <el-select v-model="curRoleId">
                    ...
                    <el-option v-for="(v,i) in roleList" :key="i" :label="v.roleName" :value="v.id">
                    </el-option>
                </el-select>
            </el-form-item>
        </el-form>
...
export default {
    ...
    methods: {
        ...
        async changeRole(){
            ...
            const res = await this.$http.put(`users/${this.userId}/role`,{rid: this.curRoleId})
            this.getUserList()
        },
        async addUser() {
